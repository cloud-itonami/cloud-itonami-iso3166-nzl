(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a New Zealand-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. New Zealand-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-nz-entity?
                                       true` (near-universal for New Zealand
                                       ...
                                       Grounded in New Zealand business
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NZBN/統一資格/GETS登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- nz-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-nz-entity? true`, INDEPENDENTLY verify
  `:has-nz-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-nz-entity?` ground truth (most New Zealand public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-nz-entity? e))
                 (not (true? (:has-nz-entity? e))))
        [{:rule :nz-entity-missing
          :detail (str subject " はニュージーランド居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- nzbn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-nzbn? true`, INDEPENDENTLY check
  `:nzbn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-nzbn? e))
                 (not (true? (:nzbn-verified? e))))
        [{:rule :nzbn-unverified
          :detail (str subject " はNZBN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (nz-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (nzbn-unverified-violations request st)
