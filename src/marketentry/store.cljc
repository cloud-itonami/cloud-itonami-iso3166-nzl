(ns marketentry.store
  "SSoT for the NZL market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems NZ" :portal "GETS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-nz-entity? true :has-nz-entity? true
             :requires-nzbn? true :nzbn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "NZL" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "GETS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-nz-entity? true :has-nz-entity? true
             :requires-nzbn? true :nzbn-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "GETS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "NZL" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "GETS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-nz-entity? true :has-nz-entity? false
             :requires-nzbn? true :nzbn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "NZL" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "GETS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-nz-entity? true :has-nz-entity? true
             :requires-nzbn? true :nzbn-verified? false
             :drafted? false :submitted? false
             :jurisdiction "NZL" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "GETS"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-nz-entity? has-nz-entity?
                               requires-nzbn? nzbn-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-nz-entity?)  (assoc :engagement/requires-nz-entity? requires-nz-entity?)
    (some? has-nz-entity?)       (assoc :engagement/has-nz-entity? has-nz-entity?)
    (some? requires-nzbn?)    (assoc :engagement/requires-nzbn? requires-nzbn?)
    (some? nzbn-verified?)    (assoc :engagement/nzbn-verified? nzbn-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-nz-entity? :engagement/has-nz-entity?
   :engagement/requires-nzbn? :engagement/nzbn-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-nz-entity? (boolean (:engagement/requires-nz-entity? m))
     :has-nz-entity? (boolean (:engagement/has-nz-entity? m))
     :requires-nzbn? (boolean (:engagement/requires-nzbn? m))
     :nzbn-verified? (boolean (:engagement/nzbn-verified? m))
