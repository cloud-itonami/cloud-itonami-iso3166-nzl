(ns marketentry.marketentryllm
  ...
  the NZL public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-nz-entity?=" (:has-nz-entity? e)
                        " nzbn-verified?=" (:nzbn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-nz-entity? e))
                              (:has-nz-entity? e))
                          (or (not (:requires-nzbn? e))
                              (:nzbn-verified? e)))
                   0.9 0.3)}))
