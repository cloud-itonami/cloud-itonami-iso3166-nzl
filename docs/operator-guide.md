...
2. Register the client's intake: business type, target public function,
   prior filing history in New Zealand if any.
3. Run the advisor in read-only mode against GETS (Government Electronic Tenders Service).
...
- named referral relationship with New Zealand-licensed counsel or a
...
a working referral relationship with New Zealand-licensed counsel or a
registered agent for whatever licensed representation the law of
New Zealand requires for actual public-procurement filings.
