# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — New Zealand

## Classification

- Repository: `cloud-itonami-iso3166-nzl`
- ISO 3166: `NZL` (New Zealand)
...
- Social impact: [:broader-outcomes-scoring-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a New Zealand
  public contract
...
- registration walkthrough for GETS (Government Electronic Tenders Service)
- business/tax registration checklist: an NZBN (New Zealand Business Number) registration; GST registration with Inland Revenue where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: no blanket local-content quota, but the Government Procurement Rules include a broader-outcomes requirement that weighs Māori business participation and regional economic development on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to New Zealand-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in New Zealand).
